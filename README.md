...
| 北京时间        | UTC             | Cron 表达式       |
| --------------- | --------------- | ----------------- |
| 工作日 08:00    | 00:00 周一–周五 | `0 0 * * 1-5`     |
| 周末 10:00       | 02:00 周日      | `0 2 * * 0`       |
| 周末 10:00       | 02:00 周六      | `0 2 * * 6`       |

三条表达式都在 `wrangler.jsonc` 的 `triggers.crons` 中，Cloudflare 会按各自时间触发 `scheduled`。代码内一律以 UTC+8 解释“今天”。
